...
<div class="ui-container">
	...
	<div class="sprite v-color-box-body">
		<a href='?_src_=prefs;preference=body_color;task=input' style="display: block; width: 100%; height: 100%;">
			<div id="voice-blob-body" class="sprite v-blob-body" style="background-color: #[normalize_skin_tone_hex(read_preference(/datum/preference/choiced/skin_tone))];"></div>
		</a>
	</div>
...
/datum/preferences/proc/update_menu_data(mob/user, list/fields_to_update)
	...
	if(update_all || ("body_color" in fields_to_update))
		params["body_color"] = "#[normalize_skin_tone_hex(read_preference(/datum/preference/choiced/skin_tone))]"
		...
		if(!race_ban)
			...
			for(var/list/category in omegalist)
				...
				for(var/datum/job/job in available_jobs)
					var/rank = job.title
					var/used_name = job.get_informed_title(user, TRUE)
