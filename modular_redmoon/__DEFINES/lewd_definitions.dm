...
/mob/living/proc/has_ears()
	var/mob/living/carbon/C = src
	if(istype(C))
		if(C.get_item_by_slot(ITEM_SLOT_EARS_LEFT))
			return HAS_UNEXPOSED_GENITAL
		else
			return HAS_EXPOSED_GENITAL
	return FALSE
			...
			if(!last_genital)
				if(has_penis())
					...
					switch(target_orifice)
						if(CUM_TARGET_MOUTH)
							if(partner.has_mouth() && (partner.mouth_is_free())) // BLUEMOON EDIT
						...
						if(CUM_TARGET_VAGINA)
							...
							if(has_vagina == TRUE || (has_vagina == HAS_EXPOSED_GENITAL)) // BLUEMOON EDIT
						...
						if(CUM_TARGET_ANUS)
							...
							if(has_anus == TRUE || (has_anus == HAS_EXPOSED_GENITAL)) // BLUEMOON EDIT
						...
						if(CUM_TARGET_PENIS)
							...
							if(has_penis == TRUE || (has_penis == HAS_EXPOSED_GENITAL)) // BLUEMOON EDIT
								message = "кончает на [partner_name]."
							else
								message = "кончает под себя!"
...
/mob/living/proc/get_shoes(singular = FALSE)
	var/obj/A = get_item_by_slot(ITEM_SLOT_SHOES)
	if(A)
		var/txt = A.name
